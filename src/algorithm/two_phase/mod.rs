...
use crate::algorithm::two_phase::matrix_provider::filter::generic_wrapper::{
    IntoFilteredColumn, RemoveRows,
};
use crate::algorithm::two_phase::matrix_provider::MatrixProvider;
use crate::algorithm::two_phase::phase_one::{
    FeasibilityComputeTrait, FullInitialBasis, Rank, RankedFeasibilityResult,
};
use crate::algorithm::two_phase::strategy::pivot_rule::SteepestDescent;
use crate::algorithm::two_phase::tableau::inverse_maintenance::{ops as im_ops, InverseMaintener};
        ...
        match self.compute_bfs_giving_im::<IM>() {
            RankedFeasibilityResult::Feasible {
                ...
                Rank::Deficient(rows_to_remove) if !rows_to_remove.is_empty() => {
                    ...
                    let mut non_artificial =
                        Tableau::<_, NonArtificial<_>>::from_artificial_removing_rows(
                            inverse_maintainer,
                            nr_artificial_variables,
                            basis,
                            &rows_removed,
                ...
                _ => {
                    let mut non_artificial_tableau =
    ...
    MP: MatrixProvider<Column: IdentityColumn + IntoFilteredColumn>,
{
        ...
        for<'r> IM::F: im_ops::Cost<MP::Cost<'r>>,
    {
        ...
        let basis_indices = basis_indices
            .into_iter()
            .map(|(_row, column)| column)
            .collect();
...
mod test {
    ...
    fn solve_matrix() {
        ...
        let result =
            SolveRelaxation::solve_relaxation::<Carry<S, LUDecomposition<S>>>(&matrix_data_form);
        //  Optimal value: R64!(4.5)
        assert_eq!(
            result,
            OptimizationResult::FiniteOptimum(SparseVector::from_test_tuples(
                vec![(1, 0.5f64), (3, 2.5f64), (4, 1.5f64),],
                5
            ))
        );
    }
    ...
    fn solve_relaxation_1() {
        ...
        let constraints = ColumnMajor::from_test_data::<T, _, _>(&[vec![1, 0], vec![1, 1]], 2);
        let b = DenseVector::from_test_data(vec![3f64 / 2f64, 5f64 / 2f64]);
        let variables = vec![
            Variable {
                cost: R64!(-2),
        ...
        let data = MatrixData::new(&constraints, &b, vec![], 0, 0, 2, 0, &variables);

        let result = data.solve_relaxation::<Carry<S, LUDecomposition<S>>>();
        assert_eq!(
            result,
            OptimizationResult::FiniteOptimum(SparseVector::from_test_tuples(
                vec![(0, 3f64 / 2f64), (1, 1f64),],
                4
            ))
        );
    }
